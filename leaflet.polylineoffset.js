...
*/
    ...
    */
		circularArc: function (s1, s2, distance) {
			...
			if (signedAngle * distance > 0 && signedAngle * this.getSignedAngle(s1.offset, [s1.offset[0], s2.offset[1]]) > 0) {
	...
	L.Polyline.include({
		_projectLatlngs: function (latlngs, result, projectedBounds) {
			...
			if (isFlat) {
				...
				// Offset management hack 2.0 (relative to zoom) ---
				if (this.options.offset) {
					const zoom = this._map.getZoom();
					const metersPerPixel = 40075016.686 / (256 * Math.pow(2, zoom)); // Equatore
					const dynamicOffset = this.options.offset / metersPerPixel;

					ring = L.PolylineOffset.offsetPoints(ring, dynamicOffset);
				}
				// Offset management hack END ---
